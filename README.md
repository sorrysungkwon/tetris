...
| v1.0.8 | **Accessibility & Visual Polish**: Colorblind mode (stripe/dot pattern overlays on all pieces). Animation intensity setting (Full / Reduced / Off) in pause menu. UI refinements — button hover states, overlay contrast, typography tweaks. Privacy & Terms footer split into two lines (links row + copyright row) to prevent awkward mobile wrapping. |
