...
| ⚠️ open bug | **iOS PWA Canvas Oversizing (Resolved & Deployed)**: On cold start, the canvas rendered too large due to late-resolving CSS `env()` variables. Both Option A (pure math estimation) and Option B (15-frame RAF polling loop) have been fully implemented, polished, and deployed with dynamic short-viewport auto-scaling down to `10px` cell sizes. Accessible at `prevglow-a.vercel.app` & `prevglow-b.vercel.app` for final real-device comparison. |
