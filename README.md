...
| ⚠️ open bug | **iOS PWA Canvas Oversizing**: On cold start, the game canvas renders larger than intended (CELL=28/gameH=560px instead of CELL=25/gameH=500px). Root cause: `env(safe-area-inset-*)` is not resolved in JS even after one RAF, so the RAF correction oversizes the canvas instead of fixing it. The `resize` skip guard then blocks self-correction. Two fix options documented in `WALKTHROUGH.md` and `TODO.md` — Option A (revert to hardcoded arithmetic) is recommended. |
